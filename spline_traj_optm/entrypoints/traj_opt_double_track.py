...
def main():
    ...
    opt_traj_d[:, Trajectory.NORM_X] = np.cos(opt_traj_d[:, Trajectory.YAW])
    opt_traj_d[:, Trajectory.NORM_Y] = np.sin(opt_traj_d[:, Trajectory.YAW])
    ...
    ca.DM(opt_traj_d.points[:, :Trajectory.NORM_Y+1]).to_file("ttl_optm.txt", "txt")
